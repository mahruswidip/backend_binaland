                        ...
                        <table id="dataTable-artikel" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Publikasi</th>
                                ...
                                @foreach ($artikel as $row)
                                    <tr>
                                        ...
                                        <td class="lebar-tabel text-center">
                                            ...
                                            <form action="{{ route('artikel.destroy', $row->id_artikel) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn bg-gradient-danger btn-sm">Hapus
                                                </button>
                                            </form>
                                            <a href="{{ route('artikel.detail', $row->id_artikel) }}" class="btn bg-gradient-success btn-sm">Tinjau
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
